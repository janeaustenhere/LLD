...
@Slf4j
@Component("twoThousandNoteDispenser")
public class TwoThousandNoteDispenser implements CashDispenser {
    ...
    public void dispense(ATM atm, Card card, BigDecimal amount) {
        ...
        int noteCount = Math.min(amount.divide(BigDecimal.valueOf(2000)).intValue(),
                atm.getTotalCountForTwoThousandsNotes());
        log.info("Two thousand rupees not dispense: count:: {}", noteCount);
        atm.setTotalCountForTwoThousandsNotes(atm.getTotalCountForTwoThousandsNotes()-noteCount);
        BigDecimal remainingAmount = amount.subtract(BigDecimal.valueOf(2000L *noteCount));
